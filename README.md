...
[금융감독원 DART Open API](https://opendart.fss.or.kr)를 연동하여 손익계산서, 재무상태표, 현금흐름표 등 공시 기반 재무제표도 조회할 수 있습니다.
...
> `DART_API_KEY`는 [DART Open API](https://opendart.fss.or.kr)에서 발급. 없으면 DART 재무제표 도구만 비활성화되고 나머지는 정상 동작합니다.
...
### 2.6 DART 재무제표 (Financial Statements)

> [DART Open API](https://opendart.fss.or.kr) 기반. `DART_API_KEY` 환경변수 필요.

- `get_dart_corp_code`: 기업명으로 DART 고유번호 검색
- `get_dart_income_statement`: 손익계산서 (매출액, 영업이익, 당기순이익 등)
- `get_dart_balance_sheet`: 재무상태표 (자산, 부채, 자본)
- `get_dart_cash_flow`: 현금흐름표
...
```
"삼성전자 2023년 손익계산서 보여줘"
"현대차 작년 재무상태표 알려줘"
"카카오 2022년 현금흐름표"
"삼성전자 DART 고유번호가 뭐야?"
```

> DART 도구 사용 전 `get_dart_corp_code`로 기업 고유번호를 먼저 조회하세요.
...
- **[httpx](https://www.python-httpx.org/)**: DART API HTTP 클라이언트
